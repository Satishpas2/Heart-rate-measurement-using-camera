...
class VidMag():
    ...
    def butter_bandpass_filter(self, data, lowcut, highcut, fs, order=5):
        ...
        low = lowcut / omega
        high = highcut / omega
        #print("low, high, frame_no",low,high,frame_no)
        self.frame_no += 1
        b, a = signal.butter(order, [low, high], btype='band')
        y = signal.lfilter(b, a, data, axis=0)
        return y
    
    def magnify_motion(self,video_tensor,fps,low=.8,high=1.2,levels=3,amplification=5):
    ...
    def buffer_to_tensor(self, buffer): 
        tensor = np.zeros((len(buffer), 192, 256), dtype = "float")
        i = 0
    ...
    def key_handler(self):
        ...
        if self.pressed == ord('m'):  # exit program on 'esc'
            print("[INFO] Exiting")
            self.webcam.stop()
            sys.exit()    
    ...
    def mainLoop(self):
        ...
        f1 = imutils.resize(gray_frame, width = 256)
        ...
        cv2.putText(gray_frame, "FPS "+str(float("{:.2f}".format(self.fps))),
        ...
        frame = cv2.cvtColor(f2, cv2.COLOR_BGR2RGB)
        cv2.imwrite("/media/satish/data/BRM/output.jpg", frame)

        # Process the frame with MediaPose
        results = mp_pose.process(frame)
        print(results.pose_landmarks)   
        # Check if pose landmarks are detected
        if results.pose_landmarks:
            # Get the shoulder and hip landmarks
            shoulder_landmark1 = results.pose_landmarks.landmark[mp.solutions.pose.PoseLandmark.LEFT_SHOULDER]
            shoulder_landmark2 = results.pose_landmarks.landmark[mp.solutions.pose.PoseLandmark.RIGHT_SHOULDER]
            hip_landmark1 = results.pose_landmarks.landmark[mp.solutions.pose.PoseLandmark.LEFT_HIP]
            hip_landmark2 = results.pose_landmarks.landmark[mp.solutions.pose.PoseLandmark.RIGHT_HIP]

            # Convert the landmark coordinates to pixel values
            shoulder_point1 = (int(shoulder_landmark1.x * frame.shape[1]), int(shoulder_landmark1.y * frame.shape[0]))
            shoulder_point2 = (int(shoulder_landmark2.x * frame.shape[1]), int(shoulder_landmark2.y * frame.shape[0]))
            hip_point1 = (int(hip_landmark1.x * frame.shape[1]), int(hip_landmark1.y * frame.shape[0]))
            hip_point2 = (int(hip_landmark2.x * frame.shape[1]), int(hip_landmark2.y * frame.shape[0]))

            # Draw circles at the shoulder and hip points
            cv2.circle(frame, shoulder_point1, 5, (0, 0, 255), -1)
            cv2.circle(frame, shoulder_point2, 5, (0, 0, 255), -1)
            cv2.circle(frame, hip_point1, 5, (0, 0, 255), -1)
            cv2.circle(frame, hip_point2, 5, (0, 0, 255), -1)

            # Track the points using the Lucas-Kanade method
