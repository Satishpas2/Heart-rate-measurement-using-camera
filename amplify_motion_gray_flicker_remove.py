...
class VidMag():
    ...
    def magnify_motion(self,video_tensor,fps,low=.2,high=.4,levels=3,amplification=3):
    ...
    def mainLoop(self):
        ...
        cv2.putText(gray_frame, "FPS "+str(float("{:.2f}".format(self.fps))),
                       (20,420), cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255),2)
        ...
        output_video = cv2.VideoWriter('output.mp4', cv2.VideoWriter_fourcc(*'mp4v'), self.fps, (f2.shape[1], f2.shape[0]))
