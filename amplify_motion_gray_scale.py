...
class VidMag():
    ...
    def magnify_motion(self,video_tensor,fps,low=.5,high=2,levels=3,amplification=10):
    ...
    def mainLoop(self):
        ...
        if len(self.p0) is 0:
            print("phir aaya!!")       
            #old_gray = cv2.cvtColor(f2, cv2.COLOR_BGR2GRAY)
            ...
            x3 = 310
            y3 = 310
            self.p0 = np.array([[x1, y1], [x2, y2], [x3, y3]], dtype=np.float32)
        ...
        cv2.moveWindow('frame', 1000, 0)  # Move the window to the rightmost corner
        # Now update the previous frame and previous points
        old_gray = frame_gray.copy()
        self.p0 = good_new.reshape(-1,1,2)
        print("type self.p0",self.p0)
        #print(f2.shape)
